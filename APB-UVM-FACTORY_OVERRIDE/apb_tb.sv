...
`include "uvm_macros.svh"
    ...
    class apb_driver extends uvm_driver #(apb_transaction);
        ...
        // declared virtual so a factory-overridden child can extend/replace it
    ...
    class apb_driver_slow extends apb_driver;
        `uvm_component_utils(apb_driver_slow)

        function new(string name = "apb_driver_slow", uvm_component parent = null);
            super.new(name, parent);
        endfunction

        task drive_transfer(apb_transaction tr);
            `uvm_info("APB_DRV_SLOW", "Using OVERRIDDEN driver (factory)", UVM_LOW)
            @(posedge vif.PCLK);      // extra bubble cycle = the visible behavior change
    ...
    class apb_agent extends uvm_agent;
        ...
        function void build_phase(uvm_phase phase);
            ...
            if (get_is_active() == UVM_ACTIVE) begin
                ...
                // NOTE: create() goes through the factory, so a type_override
                // registered before build_phase will swap this for apb_driver_slow
    ...
    // Baseline test — runs with the ORIGINAL apb_driver
    ...
    class apb_test_override extends apb_test;
        `uvm_component_utils(apb_test_override)

        function new(string name = "apb_test_override", uvm_component parent = null);
            super.new(name, parent);
        endfunction

        function void build_phase(uvm_phase phase);
            // type override: every apb_driver::create() call in this test
            // will now return an apb_driver_slow instead
